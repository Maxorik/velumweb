import React, {useEffect, useRef, useState, FC, Ref} from "react";
...
const AmbientPlayer: FC = ({type}) => {
    ...
    const playerHandler = async () => {
        ...
        if (audioRef.current) {
            const audio = await import(`../../assets/sounds/${ambientSettings[type].audio}.mp3`);
            audioRef.current.src = audio.default;
...
const ambientSettings: IAmbientSettings = {
    whitenoise: {
